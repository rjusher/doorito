...
def cleanup_expired_uploads_task(self):
    """Delete file uploads older than FILE_UPLOAD_TTL_HOURS.
    ...
    expired_qs = FileUpload.objects.filter(created_at__lt=cutoff)
    ...
    batch = FileUpload.objects.filter(pk__in=batch_pks)
    ...
    logger.info(
        "Cleaned up %d expired uploads (%d files removed), "
        "%d remaining.",
        deleted_count,
        deleted_files,
        remaining,
